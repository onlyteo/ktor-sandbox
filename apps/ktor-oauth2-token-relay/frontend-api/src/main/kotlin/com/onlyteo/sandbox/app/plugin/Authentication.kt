...
import com.onlyteo.sandbox.app.model.asAccessToken
import com.onlyteo.sandbox.app.serializer.JacksonSessionSerializer
                ...
                session<UserSession>(session.name) {
                    validate { currentSession ->
                        ...
                        if (Instant.now().isAfter(sessionExpiry)) {
                            com.onlyteo.sandbox.app.plugin.securityLogger.debug("Session expired")
                            null
                        } else if (Instant.now().isAfter(accessTokenExpiry)) {
                            com.onlyteo.sandbox.app.plugin.securityLogger.debug("Access token expired")
                            ...
                            com.onlyteo.sandbox.app.plugin.securityLogger.debug("Session verified")
                            currentSession
                        }
                    }
                    challenge {
